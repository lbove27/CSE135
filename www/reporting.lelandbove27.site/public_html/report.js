...
dashboard.href = "https://reporting.lelandbove27.site/user/dashboard/" + sessionStorage.getItem('auth_token');
...
async function createErrorsToday() {
  await fetch("https://lelandbove27.site/api/activity/getType/error", {
        ...
        let todayDate = stringDate.split('T')[0];
        data.forEach(obj => {
            if(obj.date.includes(todayDate)) {
              totalCount++;
            }
        });
        let errorsToday = document.getElementById('errors-today');
        errorsToday.innerHTML = totalCount;
    });
}
